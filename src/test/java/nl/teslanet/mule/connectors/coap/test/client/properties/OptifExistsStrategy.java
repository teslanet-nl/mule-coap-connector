 ...
 */
public class OptifExistsStrategy implements OptionStrategy
{
     ...
     */
    public OptifExistsStrategy( Boolean value )
    {
        this.value= value;
    }
    ...
    {
        if ( value ) response.getOptions().addIfMatch( new byte [0] );
    }
    ...
    public boolean validateOption( Request request )
    {
        if ( value )
        {
            return request.getOptions().getIfMatch().size() == 1 && request.getOptions().getIfMatch().get( 0 ).length == 0;
        }
        else
        {
            return request.getOptions().getIfMatch().size() == 0;
        }
    }
}
